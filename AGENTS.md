...
Only one dev server runs at a time: the QA integration server above. Start it with `bun run dev:portless` in `/Users/philip/src/philsapp-qa`; stop it with ctrl+c in its pane (not `astro dev stop`, which would leave the Portless proxy running). Under Astro 7 the server writes `.astro/dev.json` (pid/port/url) while running — `bunx astro dev status|logs` is a useful out-of-band check without reading the pane. Astro 7's agent auto-backgrounding does not trigger in our plain shell panes, so `bun run dev:portless` stays foreground as before. One caveat: if the QA server is ever started from within an agent session (not via `herdr pane run` into the plain shell pane), Astro may auto-background, Portless will tear down, and the URL will die — in that case force foreground with `ASTRO_DEV_BACKGROUND=0 bun run dev:portless`.
